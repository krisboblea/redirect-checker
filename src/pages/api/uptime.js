...
async function fetchData(serviceFilter = null) {
  try {
    const allSites = JSON.parse(UPTIME_SITES);

    // Filter services if specified, otherwise use all
    const sites = serviceFilter && serviceFilter.length > 0
      ? allSites.filter(token => serviceFilter.includes(token))
      : allSites;
...
async function handler(req, res) {
  try {
    ...
    // Priority 3: Use environment variable as fallback
    else if (UPTIME_SITES) {
      try {
        serviceFilter = JSON.parse(UPTIME_SITES);
      } catch (e) {
        console.error("Error parsing UPTIME_SITES:", e);
      }
    }
